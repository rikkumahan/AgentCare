...
git commit -m "Add Docker Compose for local Postgres + app container"
```

(`app` service won't fully start yet — `app/main.py` and Alembic don't exist
until later tasks. That's expected; only `db` needs to be up for now.)
...
- Create: `alembic/versions/` (generated migration file goes here)
...
- [ ] **Step 4: Create the `alembic/versions/` directory**

```bash
mkdir -p alembic/versions
```

- [ ] **Step 5: Drop the test-created schema so autogenerate sees an empty database**
...
- [ ] **Step 8: Verify the schema matches — run the model test suite against the Alembic-created schema**
...
git commit -m "Add RBAC-protected patient and staff dashboard routes"
```
